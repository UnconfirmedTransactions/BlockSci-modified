...
namespace blocksci {
    ...
    template <>
    class BLOCKSCI_EXPORT ScriptAddress<AddressType::WITNESS_TAPROOTHASH> : public ScriptHashBase {
    public:
        constexpr static AddressType::Enum addressType = AddressType::WITNESS_TAPROOTHASH;
...
namespace std {
    ...
    template<> struct BLOCKSCI_EXPORT hash<blocksci::ScriptAddress<blocksci::AddressType::WITNESS_TAPROOTHASH>> {
        size_t operator()(const blocksci::ScriptAddress<blocksci::AddressType::WITNESS_TAPROOTHASH> &address) const {
            std::size_t seed = 7653463;
            blocksci::hash_combine(seed, static_cast<const blocksci::ScriptHashBase &>(address));
            return seed;
        }
    };
