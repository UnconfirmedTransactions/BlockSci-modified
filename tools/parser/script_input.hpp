...
struct InputView {
    ...
    bool taprootActivated;
    
    InputView(uint32_t inputNum_, uint32_t txNum_, std::vector<WitnessStackItem> && witnessStack_, bool witnessActivated_, bool taprootActivated_) : inputNum(inputNum_), txNum(txNum_), witnessStack(std::move(witnessStack_)), witnessActivated(witnessActivated_), taprootActivated(taprootActivated_) {}
};
...
template<>
struct ScriptInputData<blocksci::AddressType::Enum::WITNESS_TAPROOTHASH> : public ScriptInputDataBase {
    AnyScriptOutput wrappedScriptOutput;
    std::unique_ptr<AnyScriptInput> wrappedScriptInput;
    
    ScriptInputData(const InputView &inputView, const blocksci::CScriptView &scriptView, const RawTransaction &tx, const SpendData<blocksci::AddressType::Enum::WITNESS_TAPROOTHASH> &);
