...
void AddressWriter::serializeOutputImp(const ScriptOutput<AddressType::WITNESS_TAPROOTHASH> &output, ScriptFile<blocksci::DedupAddressType::SCRIPTHASH> &file, bool topLevel) {
    auto data = file[output.scriptNum - 1];
    data->hash256 = output.data.hash;
    data->isSegwit = true;
    data->saw(AddressType::WITNESS_TAPROOTHASH, topLevel);
}
...
void AddressWriter::serializeInputImp(const ScriptInput<AddressType::WITNESS_TAPROOTHASH> &input, ScriptFile<DedupAddressType::SCRIPTHASH> &file) {
    auto data = file[input.scriptNum - 1];
    data->wrappedAddress = input.data.wrappedScriptOutput.address();
}
...
void AddressWriter::serializeWrappedScript(const ScriptInputData<AddressType::Enum::WITNESS_TAPROOTHASH> &data, uint32_t txNum, uint32_t outputTxNum) {
    if (data.wrappedScriptOutput.isNew()) {
        serializeNewOutput(data.wrappedScriptOutput, txNum, false);
    } else {
        serializeExistingOutput(data.wrappedScriptOutput, false);
