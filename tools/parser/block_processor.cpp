...
class BlockFileReader<FileTag> : public BlockFileReaderBase {
    ...
    void nextTxImp(RawTransaction *tx, bool isSegwit, bool isTaproot) {
        try {
            auto firstTxOffset = reader->offset();
            tx->load(*reader, currentTxNum, currentHeight, isSegwit, isTaproot);
            if (shouldAdvance) {
                currentTxNum++;
            } else {
...
public:
    ...
    void nextTx(RawTransaction *tx, bool isSegwit, bool isTaproot) override {
        nextTxImp<true>(tx, isSegwit, isTaproot);
    }
    
    void nextTxNoAdvance(RawTransaction *tx, bool isSegwit, bool isTaproot) override {
        nextTxImp<false>(tx, isSegwit, isTaproot);
    }
...
class BlockFileReader<RPCTag> : public BlockFileReaderBase {
    ...
    void nextTxImp(RawTransaction *tx, bool isSegwit, bool isTaproot) {
        if (currentHeight == 0) {
            ...
            tx->load(txinfo, firstTxNum + currentTxOffset, currentHeight, isSegwit, isTaproot);
        }
        if (shouldAdvance) {
            currentTxOffset++;
        }
...
blocksci::RawBlock readNewBlock(uint32_t firstTxNum, uint64_t firstInputNum, uint64_t firstOutputNum, const BlockInfoBase &block, BlockFileReaderBase &fileReader, NewBlocksFiles &files, const std::function<bool(RawTransaction *&tx)> &loadFunc, const std::function<void(RawTransaction *tx)> &outFunc, bool isSegwit, bool isTaproot) {
    ...
    for (uint32_t j = 0; j < block.nTx; j++) {
        ...
        fileReader.nextTx(tx, isSegwit, isTaproot);
...
std::vector<std::function<void(RawTransaction &tx)>> GenerateScriptOutputsStep::steps() {
    return {[&](RawTransaction &tx) {
        ...
        for (auto &output : tx.outputs) {
            ...
            tx.scriptOutputs.emplace_back(output.getScriptView(), true, tx.isSegwit, tx.isTaproot);
        }
    }};
}
...
std::vector<std::function<void(RawTransaction &tx)>> GenerateScriptInputStep::steps() {
    return {[&](RawTransaction &tx) {
        ...
        uint16_t i = 0;
        for (auto &input : tx.inputs) {
            InputView inputView(i, tx.txNum, input.getWitnessStack(), tx.isSegwit, tx.isTaproot);
...
std::vector<blocksci::RawBlock> BlockProcessor::addNewBlocks(const ParserConfiguration<ParseTag> &config, std::vector<BlockInfo<ParseTag>> blocks, UTXOState &utxoState, UTXOAddressState &utxoAddressState, AddressState &addressState, UTXOScriptState &utxoScriptState) {
    ...
    auto importer = std::async(std::launch::async, [&] {
        ...
        for (auto &block : blocks) {
            ...
            auto newBlock = readNewBlock(currentTxNum, currentInputNum, currentOutputNum, block, fileReader, files, loadFinishedTx, outFunc, block.height >= config.dataConfig.chainConfig.segwitActivationHeight, block.height >= config.dataConfig.chainConfig.taprootActivationHeight);
