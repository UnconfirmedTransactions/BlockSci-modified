...
ScriptOutputDataType extractScriptData(const blocksci::CScriptView &scriptPubKey, bool p2shActivated, bool witnessActivated, bool taprootActivated) {
    ...
    if (witnessActivated && scriptPubKey.IsWitnessProgram(witnessversion, witnessprogram)) {
        ...
        } else if (witnessversion == 1 && taprootActivated){
            if (witnessprogram.size() == 32){
                return ScriptOutputData<AddressType::Enum::WITNESS_TAPROOTHASH>(uint256{witnessprogram.begin(), witnessprogram.end()});
            }else{
                // taproot with other script length is treated as nonstandard
                return ScriptOutputData<AddressType::Enum::NONSTANDARD>{scriptPubKey};
            }
        }else {
...
AnyScriptOutput::AnyScriptOutput(const blocksci::CScriptView &scriptPubKey, bool p2shActivated, bool witnessActivated, bool taprootActivated) : wrapped(mpark::visit(ScriptOutputGenerator(), extractScriptData(scriptPubKey, p2shActivated, witnessActivated, taprootActivated))) {}
...
blocksci::uint160 ScriptOutputData<blocksci::AddressType::Enum::WITNESS_TAPROOTHASH>::getHash() const {
    return ripemd160(reinterpret_cast<const char *>(&hash), sizeof(hash));
}

blocksci::ScriptHashData ScriptOutputData<blocksci::AddressType::Enum::WITNESS_TAPROOTHASH>::getData(uint32_t txNum, bool topLevel) const {
    blocksci::ScriptHashData data{txNum, hash, blocksci::RawAddress{0, blocksci::AddressType::Enum::NONSTANDARD}};
    data.saw(blocksci::AddressType::Enum::WITNESS_TAPROOTHASH, topLevel);
    return data;
}
// MARK: TX_MULTISIG
