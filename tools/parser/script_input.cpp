...
std::pair<AnyScriptOutput, std::unique_ptr<AnyScriptInput>> p2shGenerate(const InputView &inputView, const blocksci::CScriptView &scriptView, const RawTransaction &tx, const SpendData<blocksci::AddressType::Enum::SCRIPTHASH> &) {
    ...
    AnyScriptOutput wrappedScriptOutput(wrappedOutputScript, false, inputView.witnessActivated, inputView.taprootActivated);
...
std::pair<AnyScriptOutput, std::unique_ptr<AnyScriptInput>> p2shWitnessGenerate(const InputView &inputView, const blocksci::CScriptView &scriptView, const RawTransaction &tx, const SpendData<blocksci::AddressType::Enum::WITNESS_SCRIPTHASH> &) {
    ...
    AnyScriptOutput wrappedScriptOutput(witnessView, false, inputView.witnessActivated, inputView.taprootActivated);
...
std::pair<AnyScriptOutput, std::unique_ptr<AnyScriptInput>> p2shWitnessGenerate(const InputView &inputView, const blocksci::CScriptView &scriptView, const RawTransaction &tx, const SpendData<blocksci::AddressType::Enum::WITNESS_TAPROOTHASH> &) {
    assert(inputView.witnessStack.size() > 0);
    auto &witnessScriptItem = inputView.witnessStack.back();
    auto outputBegin = reinterpret_cast<const unsigned char *>(witnessScriptItem.itemBegin);
    blocksci::CScriptView witnessView(outputBegin, outputBegin + witnessScriptItem.length);
...
ScriptInputData<blocksci::AddressType::Enum::WITNESS_TAPROOTHASH>::ScriptInputData(std::pair<AnyScriptOutput, std::unique_ptr<AnyScriptInput>> data) : wrappedScriptOutput(data.first), wrappedScriptInput(std::move(data.second)) {
}
...
ScriptInputData<blocksci::AddressType::Enum::WITNESS_TAPROOTHASH>::ScriptInputData(const InputView &inputView, const blocksci::CScriptView &scriptView, const RawTransaction &tx, const SpendData<blocksci::AddressType::Enum::WITNESS_TAPROOTHASH> &spendData) : ScriptInputData(p2shWitnessGenerate(inputView, scriptView, tx, spendData)) {}
...
void ScriptInputData<blocksci::AddressType::Enum::WITNESS_TAPROOTHASH>::process(AddressState &state) {
    uint32_t scriptNum = wrappedScriptOutput.resolve(state);
    wrappedScriptInput->process(state);
    wrappedScriptInput->setScriptNum(scriptNum);
}
