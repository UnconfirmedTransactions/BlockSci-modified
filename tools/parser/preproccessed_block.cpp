...
void RawTransaction::load(SafeMemReader &reader, uint32_t txNum_, blocksci::BlockHeight blockHeight_, bool witnessActivated, bool taprootActivated) {
    txNum = txNum_;
    isSegwit = witnessActivated;
    isTaproot = taprootActivated;
...
void RawTransaction::load(const getrawtransaction_t &txinfo, uint32_t txNum_, blocksci::BlockHeight blockHeight_, bool witnessActivated, bool taprootActivated) {
    txNum = txNum_;
    isSegwit = witnessActivated;
    isTaproot = taprootActivated;
    blockHeight = blockHeight_;
