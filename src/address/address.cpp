    ...
    ranges::optional<Address> getAddressFromString(const std::string &addressString, DataAccess &access) {
        if (addressString.compare(0, access.config.chainConfig.segwitPrefix.size(), access.config.chainConfig.segwitPrefix) == 0) {
            ...
            if (decoded.first == 0) {
                ...
                //witness_taproot 
                uint256 taprootHash(decoded.second.begin(), decoded.second.end());
                ranges::optional<uint32_t> addressNum = access.getHashIndex().getTaprootHashIndex(taprootHash);
                if (addressNum) {
                    return Address{*addressNum, AddressType::WITNESS_TAPROOTHASH, access};
                }
            }
            return ranges::nullopt;
        }
