...
namespace blocksci {
    ...
    using script::WitnessTaprootHash;
    ...
    WitnessTaprootHash::ScriptAddress(uint32_t addressNum_, DataAccess &access_) : ScriptAddress(addressNum_, access_.getScripts().getScriptData<dedupType(addressType)>(addressNum_), access_) {}
    ...
    std::string WitnessTaprootHash::addressString() const {
        std::vector<uint8_t> witprog;
        auto addressHash = getAddressHash();
        witprog.insert(witprog.end(), reinterpret_cast<const uint8_t *>(&addressHash), reinterpret_cast<const uint8_t *>(&addressHash) + sizeof(addressHash));
    ...
    std::string WitnessTaprootHash::toString() const {
        std::stringstream ss;
        ss << "WitnessTaprootHashAddress(" << addressString() << ")";
        return ss.str();
    }
    
    std::string WitnessTaprootHash::toPrettyString() const {
        std::stringstream ss;
        ss << "WitnessTaprootHashAddress(" << addressString();
        ss << ", wrapped_address=";
        auto wrapped = wrappedScript();
